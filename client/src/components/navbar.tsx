...
import { AUTH_SUCCESS_EVENT } from '@/lib/AuthContext';
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
...
export default function Navbar({ mobileMenuOpen, setMobileMenuOpen }: NavbarProps) {
  ...
  useEffect(() => {
    const handleAuthSuccess = () => {
      // Close the login dialog when authentication is successful
    ...
    <>
      ...
      {showLoginDialog && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative animate-in fade-in zoom-in">
            <button 
            ...
            <h3 className="text-2xl font-bold mb-6 text-center">
              <span className="bg-gradient-to-r from-[#8A2387] via-[#E94057] to-[#F27121] text-transparent bg-clip-text">Sign In</span>
            </h3>
            <LoginButton showAll={true} className="w-full" />
          </div>
        </div>
      )}
